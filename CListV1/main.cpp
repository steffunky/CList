...
namespace
{
    ...
    bool areListsEquals(const CList<T> &list1, const CList<T> &list2)
    {
        ...
        CNode<T> elem1;
        CNode<T> elem2;
        for (elem1 = list1.GetFirstNode(), elem2 = list2.GetFirstNode();
              !(elem1.GetData() == list1.GetLastNode()->GetSuivant()->GetData());
    ...
    void afficherList(const CList<T> & List) noexcept
    {
        ...
        for (std::shared_ptr<CNode<T>> Ptr(List.GetFirstNode()); Ptr != List.GetLastNode()->GetSuivant(); Ptr = Ptr->GetSuivant())
        {
            cout << Ptr->GetData() << "; ";
        }
        cout << "\nfin de la liste \n\n" << endl;
